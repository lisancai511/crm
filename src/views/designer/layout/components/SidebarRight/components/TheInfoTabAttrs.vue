      ...
      <el-row>
        <el-col :span="24">
          <el-select
            ...
            v-model="data.defaultValue" placeholder="请选择">
            <el-option
...
import { IField, layoutComponents, tabDetailsComponents } from '@/views/designer/config/components'
...
export default class TheInfoTabAttrs extends Mixins(edit) {
  ...
  get tabDetailsComponents (): IField[] {
  ...
  getComponentByType (components: IField[]): any {
    return components.reduce((pre: any, cur: IField) => {
      pre[cur.type] = cur
      return pre
    }, {})
  }
  ...
  confirmEdit (): void {
    // 过滤删除项
    this.data.children = (this.data.children as []).filter((item: IField) => this.checkList.includes(item.type))
    // 追加新增项
    ...
    // 更新defaultValue
    if (!this.checkList.includes(this.data.defaultValue as string)) {
      this.data.defaultValue = this.checkList[0]
    }
