      ...
      <el-col class="m-t-20 user_main_col" v-for="item in tableData" :key="item.id" :span="4">
        <el-card :body-style="{padding:0}" class="user_main_col_card" style="min-height:300px">
          <div class="main">
            <el-avatar
              :size="60"
              :src="item.headImages || 'https://cube.elemecdn.com/0/88/03b0d39583f48206768a7534e55bcpng.png'"/>
            <h2>{{item.name}}</h2>
          ...
          <div class="footer">
            <el-row style="width:100%; height:100%">
              ...
              <el-col @click.native="deleteUser(item)" class="footer_mid dd-click"
                      :class="!item.deleted?'footer_mid dd-click':'footer_mid dd-click active'" :span="8">
                {{!item.deleted?'停用':'启用'}}
              </el-col>
