<template>
  <div class="singleLine">
    ...
    <OneStep :saveData="formLabelAlign" v-if="OptionList==='two'"></OneStep>
  </div>
</template>
<script lang="ts">
...
export default class OptionList extends Vue {
  private formLabelAlign = {
    name: 'li', region: '13', type: ''
  }
  @missionModule.Mutation('changeTab') changeTab:any
  @missionModule.State('OptionList') OptionList:any

  changeTabs () {
    this.$store.commit('daoda/tabs/changeTab', 'OptionList')
    this.changeTab('OptionList')
  }
}
</script>
